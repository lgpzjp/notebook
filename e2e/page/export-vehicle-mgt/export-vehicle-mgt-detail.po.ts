...
export class VehicleMgtDetail {
  modal: ElementFinder;
  nameEleModal: string;

  outputDiv: BlComboBox;
  targetPeriodDiv: BlComboBox;
  exportInfoTypeDiv: BlComboBox;
  organizationDiv: BlComboBox;
  ...
  constructor(el: ElementFinder) {
    this.modal = el;
    this.outputDiv = new BlComboBox($('bl-select#outputVehicleMgt'));
    this.targetPeriodDiv = new BlComboBox($('bl-select#targetPeriodVehicleMgt'));
    this.exportInfoTypeDiv = new BlComboBox($('bl-select#exportInfoVehicleMgt'));
    this.organizationDiv = new BlComboBox($('share-organization-combo-box#organizationVehicleMgt'));
  }
  /**
   * 表示モードの確認
   */
  ...
  getChoiceText(index: number) {
    return this.modal.$('bl-select#targetPeriodVehicleMgt').$$('li').get(index).$('button.dropdown-item').isPresent();
  }
   ...
   * @param index
   */
  clickBtn(tag: string,  id: string, index: number) {
    this.modal.$(tag + '#' + id).$$('li').get(index).$('button.dropdown-item').click();
  }
