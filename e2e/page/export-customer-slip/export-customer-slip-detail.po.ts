...
export class CustomerSlipDetail {
  modal: ElementFinder;
  nameEleModal: string;
   ...
   * @param string
   */
  ClickElementisPresent(tag: string, id: string) {
    this.modal.$$(tag + '#' + id).isPresent();
  }
   ...
   * @param index
   */
  isEnabledDate(nameClass: string, index: number) {
    return this.modal.$$('.' + nameClass + ' input').get(index).isEnabled();
  }
   ...
   * @param index
   */
  isPresentDate(nameClass: string, index: number) {
    return this.modal.$$('.' + nameClass + ' input').get(index).isPresent();
  }
